...
const DOM_V_ZAL_NAV_OVERRIDES = [
  ...
  { label: "ОТЗЫВЫ", href: "/online#results", target: "_blank" },
];
...
function WeeksCard({
  ...
  return (
    ...
    >
      ...
      {!compact && (
        <>
          <p className="mt-1 text-xs leading-snug text-stone-400">4 недели дома<br />8 недель в зале</p>
          <div className={`border-t border-[#E8E2D9] ${dividerSpacing}`}>
            <p className="text-xs leading-snug text-stone-500">
              Мягкий переход<br />
              <span className="font-medium text-stone-700">к уверенным<br />тренировкам</span>
            </p>
          </div>
        </>
      )}
      {compact && (
        <p className="mt-1 text-[10px] leading-snug text-stone-500">4 недели дома + 8 в зале</p>
      )}
    </div>
  );
}
...
const heroAudience = [
  "Для тех, кто ни разу не занимался в зале и не знает, с чего начать",
  "Для тех, кто хочет сначала освоить базу дома, без стресса и чужих взглядов",
  "Для тех, кому нужна понятная техника: гантели, тренажёры, базовые движения",
  "Для тех, кто хочет прийти в зал уже с уверенностью и планом",
] as const;

const forWhom = [
  "Хотите начать тренироваться, но зал пока кажется непонятным",
  "Не уверены, как правильно делать упражнения и брать рабочий вес",
  "Хотите сначала спокойно выстроить технику дома",
  "Устали от хаотичных тренировок без системы и результата",
  "Готовы уделять себе 30–60 минут 3–5 раз в неделю",
];

const results12 = [
  "Понимание базовых движений и техники",
  "Уверенность в работе с гантелями и тренажёрами",
  "План тренировок в зале без растерянности",
  "Более подтянутое тело, сила и выносливость",
  "Привычка тренироваться регулярно и безопасно",
];
        ...
        <div className="flex flex-col lg:h-full">
          <div>
            ...
            <p className="mt-3 text-sm font-medium tracking-wider text-[#C4956A]">
              — 4 недели дома + 8 недель в зале
            </p>
            <p className="mt-4 max-w-lg text-base leading-relaxed text-stone-500">
              Мягкий переход для женщин, которые хотят начать с нуля: сначала вы спокойно осваиваете технику дома, а затем приходите в зал уже с понятным планом, уверенностью и знанием, как работать с весами.
            </p>
          </div>
          <HeroAudienceCard
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-8 lg:grid-cols-[160px_1fr_1px_1fr_160px] lg:items-stretch">
            ...
            <div>
              <h3 className="text-xl font-bold text-stone-900">К финалу 12 недель вы:</h3>
              <ul className="mt-6 space-y-3">
                {results12.map((t) => (
                  <li key={t} className="flex gap-3 text-sm text-stone-600"><Check />{t}</li>
                ))}
